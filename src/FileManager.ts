...
export namespace FileManager {
  export function checkShortcutPath(shortcutPath: string): boolean {
    ...
    if (!fs.existsSync(shortcutPath)) {
      vscode.window.showErrorMessage("VSC Icon: The provided shortcut path does not exist.");
      return false;
    }
    if (path.extname(shortcutPath).toLowerCase() !== ".lnk") {
      vscode.window.showErrorMessage("VSC Icon: The provided shortcut path is not a valid shortcut file.");
      return false;
    }

    const scData = Powershell.getShortcutData(shortcutPath);
    if (!scData) {
      vscode.window.showErrorMessage("VSC Icon: Failed to get shortcut data.");
      return false;
    }
    ...
    if (!found) {
      vscode.window.showErrorMessage("VSC Icon: The shortcut path does not point to a valid Visual Studio Code executable.");
      return false;
    }
  ...
  export function checkIconPath(iconPath: string): boolean {
    ...
    if (!fs.existsSync(iconPath)) {
      vscode.window.showErrorMessage("VSC Icon: The provided icon path does not exist.");
      return false;
    }
    if (path.extname(iconPath).toLowerCase() !== ".ico") {
      vscode.window.showErrorMessage("VSC Icon: The provided icon path is not a valid icon file.");
      return false;
    }
    return true;
  }
